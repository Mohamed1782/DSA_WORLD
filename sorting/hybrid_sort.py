...
class HybridSortGame:
    def __init__(self, master, go_back_callback):
        ...
        self.master.title("DSA World - Hybrid Sort")
        ...
        self.THRESHOLD = 10  # Switch to insertion sort for subarrays <= 10 elements
    ...
    def create_ui(self):
        ...
        # Info label
        info_text = f"Insertion Sort (≤{self.THRESHOLD} elements) + Merge Sort (>{self.THRESHOLD} elements)"
        info_label = tk.Label(
            self.main_frame,
            text=info_text,
            font=("Courier", 10),
        ...
        instruction_text = (
            "INSTRUCTIONS:\n"
            "Uses Insertion Sort for small subarrays (≤10) and Merge Sort for larger ones\n"
            "This is how real-world hybrid algorithms like Timsort work!"
        )
    ...
    def start_sort(self):
        """Start the hybrid sort animation"""
        if not self.array:
        ...
        self.hybrid_sort_animate(0, len(self.array_copy) - 1)
    ...
    def hybrid_sort_animate(self, left, right):
        """Hybrid sort: use insertion sort for small subarrays, merge sort for larger"""
        if right - left <= self.THRESHOLD:
            # Use insertion sort for small subarrays
            self.status_label.config(text=f"Using INSERTION SORT for [{left}, {right}]", fg=self.COLORS["orange"])
            self.insertion_sort_range(left, right)
        else:
            # Use merge sort for larger subarrays
            self.merge_sort_range(left, right)

    def insertion_sort_range(self, left, right):
        """Insertion sort for a range of array"""
        for i in range(left + 1, right + 1):
            key = self.array_copy[i]
            j = i - 1
            ...
            while j >= left and self.array_copy[j] > key:
                ...
                self.array_copy[j + 1] = self.array_copy[j]
                j -= 1

            self.array_copy[j + 1] = key
            self.array_colors[j + 1] = self.COLORS["orange"]
            self.array = self.array_copy.copy()
            self.draw_array()
            self.update_timer_display()
            time.sleep(0.1)

    def merge_sort_range(self, left, right):
        """Merge sort for a range of array"""
        if left < right:
            mid = (left + right) // 2

            # Highlight dividing
            self.highlight_range(left, right, self.COLORS["cyan"])
            self.status_label.config(text=f"Using MERGE SORT: Dividing [{left}, {right}]", fg=self.COLORS["cyan"])
            self.array = self.array_copy.copy()
            self.draw_array()
            self.update_timer_display()
            time.sleep(0.15)
            ...
            # Recursively sort
            self.merge_sort_range(left, mid)
            self.merge_sort_range(mid + 1, right)

            # Merge
            self.merge_animate_range(left, mid, right)

    def merge_animate_range(self, left, mid, right):
